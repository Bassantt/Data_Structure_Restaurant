...
void GUI::PrintMessage(string msg,string msg1,string msg2,string msg3,string msg4) const	//Prints a message on status bar
{
	...
	pWind->DrawString(10, WindHeight - (int) (145), msg);
	pWind->DrawString(10, WindHeight - (int) (120), msg1);
	pWind->DrawString(10, WindHeight - (int) (90), msg2);
	pWind->DrawString(10, WindHeight - (int) (60), msg3);
	pWind->DrawString(10, WindHeight - (int) (30), msg4);
...
void GUI::rearange()
{
	int count = 0; double p; int s=0;
	PriQueue<Order*, double>d;
	Order* Array[MaxPossibleOrdCnt];
	for (int i = 0; i<OrderCount; i++)
	{
		if (OrdListForDrawing[i] == nullptr)continue;
		if (OrdListForDrawing[i]->GetType() == TYPE_VIP)
		{
			p =OrdListForDrawing[i]->gettotalmoney() /( OrdListForDrawing[i]->getarrivaltime()* OrdListForDrawing[i]->GetDistance());
			d.enqueue(OrdListForDrawing[i],p);
			s++;
		}
	...
	{
		if (OrdListForDrawing[i] == nullptr)continue;
		if (OrdListForDrawing[i]->GetType() == TYPE_FROZ)
		{
			Array[count++] = OrdListForDrawing[i];
		}
	}
	for (int i = 0; i<OrderCount; i++)
	{
		if (OrdListForDrawing[i] == nullptr)continue;
		if (OrdListForDrawing[i]->GetType() == TYPE_NRM)
		{
			Array[count++] = OrdListForDrawing[i];
		}
	}
	ResetOrderDrawingList();
	for (int i = 0; i<s; i++)
	{
		d.dequeue(OrdListForDrawing[i]);
	}
	for (int i = 0; i < count; i++)
	{
		if (Array[i] == nullptr)continue;
		OrdListForDrawing[s++] = Array[i];

	}
	OrderCount = s;
}

void GUI::ResetOrderDrawingList()
{
	
for(int i=0;i<OrderCount;i++)
{
if(OrdListForDrawing[i]==nullptr)continue;
OrdListForDrawing[i]=nullptr;
}
...
void GUI::Deldraw(Order *o)
{
for(int i=0;i<OrderCount;i++)
{
if(OrdListForDrawing[i]==nullptr)continue;
if(OrdListForDrawing[i]->GetID()==o->GetID())
{
OrdListForDrawing[i]=nullptr;
UpdateInterface();
}
...
void GUI::DrawOrders() const
{
	...
	for(int i=0; i<OrderCount; i++)
	{
		if(OrdListForDrawing[i]==nullptr)continue;
		int orderRegion ;
		if(OrdListForDrawing[i]->GetRegion()==A_REG)orderRegion=0;
		else if(OrdListForDrawing[i]->GetRegion()==B_REG)orderRegion=1;
		else if(OrdListForDrawing[i]->GetRegion()==C_REG)orderRegion=2;
		else if(OrdListForDrawing[i]->GetRegion()==D_REG)orderRegion=3;
